...
module.exports = class Conga {
    ...
    registerProjectCommands() {
		...
		const configPath = path.join(this.currentPath, 'app', 'config', 'bundles.yml');
		const config  = yaml.parse(fs.readFileSync(configPath, 'utf-8'));

		let bundles = config.bundles.all;

        if (typeof config.bundles[this.environment] !== 'undefined') {
            bundles.concat(config.bundles[this.environment]);
        }
    ...
    registerCommand(command, file, context = 'local') {
        ...
        cmd.description(command.command);
